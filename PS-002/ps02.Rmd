...
education+ i_female + i_citizen+
  I(education^2) +
 education:i_female + i_citizen:education + i_citizen:i_female + i_citizen:i_female:education,
  data = ps_df
)
# Results
est11 %>% tidy()
...
**Answer:** The expected income for women with 16 years of education is approximately `r sum(est16$coefficients * c(1,16,1,0,16)) %>% magrittr::multiply_by(1e4) %>% scales::dollar(1)`. The expected income for men with 16 years of education is approximately `r sum(est16$coefficients * c(1,16,0,0,0)) %>% magrittr::multiply_by(1e4) %>% scales::dollar(1)`.
...
.footnote[.pink[†] There are a LOT of ways to estimate heteroskedasticity using fgls - this is only one of them]
...
```{r question 22 answer, echo = F, eval = T}
