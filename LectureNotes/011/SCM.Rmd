...
So let's learn about .pink[graphs] `r emo::ji('graph')`
...
```{r, graph-ex-undirected, echo = F, fig.height = 2.5, fig.width = 2.5}
# Plot the full DAG
ggplot(
  data = graph_dt,
...
.note[Building block 5:] .b.slate[Colliders]
...
.footnote[.pink[†] Yes, this is a stupid term, and I hate it. I way prefer just using 'collider']
...
.note[Building block 5:] .b.slate[Colliders with conditions]
...
```{r, ex-collider-bias-1, echo = F, include = F}
collider_curve = tibble(
  x = c(-1, -.2, 1),
  y = c(-0.7, 0, -0.7)
) %>% spline(n = 101) %>% as.data.table()
...
```{r dagdiag_collex, echo = F, fig.height = 2.9, fig.width = 5}

ggplot(
  data = bb5_dt2 %>% dplyr::mutate(
    name = fcase(
      name == "A", "O",
      name == "B", "C",
      name == "C", "A",
...
```{r, ex-collider-bias-1b, echo = F, fig.height = 2.9, fig.width = 5}
ggplot(
  data = bb5_dt2 %>% dplyr::mutate(
    name = fcase(
      name == "A", "O",
...
```{r, ex-collider-bias-2, echo = F, fig.height = 2.9, fig.width = 5}
ggplot(
  data = bb5_dt2 %>% dplyr::mutate(
    name = fcase(
      name == "A", "O",
...
geom_text(
  ...
  family = "Fira Sans Medium",
  size = 8,
  color = c(purple, purple, "white", "white", "white"),
  fontface = "bold"
) +
theme_void() +
theme(
...
```{r, ex-2-fig-prexp, echo = F, fig.height = 3, fig.width = 6}
# Plot the full DAG
ggplot(
  data = ex2,
  aes(x = x, y = y, xend = xend, yend = yend)
...
```{r, ex-2-fig-pstexp, echo = F, fig.height = 3, fig.width = 6}
# Plot the full DAG
ggplot(
  data = ex2,
  aes(x = x, y = y, xend = xend, yend = yend)
...
.less-right[
```{r, graph-prob-2b, echo = F, fig.height = 3, fig.width = 3}
ex_graph
```
]
...
.left-col[
```{r, graph-factorizeb, echo = F, fig.height = 3, fig.width = 3}
ex_graph
```
]
