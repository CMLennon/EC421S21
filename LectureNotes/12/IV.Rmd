...
dag_dt[, `:=`(
  ...
  path2 = (name == "D" & to == "Sel") | (name == "Sel" & to == "Y") | (name == "Y")
)]
# Shorten segments
mult = 0.15
dag_dt[, `:=`(
...
```{r, dag-ex-setup-exp, echo = F, include = F}
# The full DAG
dag_full = dagify(
  Y ~ D,
  Y ~ Sel,
...
```{r, dag-ex-exp, echo = F, fig.height = 3, fig.width = 6}
# Plot the full DAG
ggplot(
  data = dag_dt,
  aes(x = x, y = y, xend = xend, yend = yend)
...
```{r, dag-iv-setup, echo = F, include = F}
# The full DAG
dag_full = dagify(
  Y ~ D,
  Y ~ Sel,
...
```{r, dag-ex-iv, echo = F, fig.height = 3, fig.width = 6}
# Plot the full DAG
ggplot(
  data = dag_dt,
  aes(x = x, y = y, xend = xend, yend = yend)
...
```{r, dag-iv-setup-2, echo = F, include = F}
# The full DAG
dag_full = dagify(
  Y ~ D,
  Y ~ Sel,
...
```{r, dag-ex-iv-2, echo = F, fig.height = 3, fig.width = 6}
# Plot the full DAG
ggplot(
  data = dag_dt,
  aes(x = x, y = y, xend = xend, yend = yend)
...
p_load(wooldridge)
wage_df <- wage2 %>% transmute(wage, education = educ, education_dad = feduc, education_mom = meduc) %>% na.omit() %>% as_tibble() %>% data.table()
# Print to screen
wage_df %>%  DT::datatable(options = list(pageLength = 4))
```
---
...
pagedown::chrome_print("IV.html", output = "12-IV.pdf")
pagedown::chrome_print("IV.html", output = "IV.pdf")
```
