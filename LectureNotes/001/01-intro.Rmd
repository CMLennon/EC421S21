...
We can test these hypotheses __using a regression model__. After the midterm, we'll revisit this example.
...
3\. R is very __flexible and powerful__—adaptable to nearly any task, _e.g._, 'metrics, spatial data analysis, machine learning, web scraping, data cleaning, website building, teaching. As an example - all of these notes all came out of R.
...
anim_save(gg,
  ...
  units = "in",
  res = 150,
  nframes = 56,renderer = gifski_renderer(), path = dir_slides, filename = "ex_gganimate.gif")
```
...
leaflet() %>%
  addTiles() %>%
  addMarkers(lng = -123.0783, lat = 44.0443, popup = "The University of Oregon: PLC")
```
...
- __Note:__ Having a copy of R on your own computer will likely be imperative for homework, projects, _etc._ - at least until we return to in-person coursework (fingers crossed.)
