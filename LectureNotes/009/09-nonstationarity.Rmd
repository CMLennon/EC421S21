...
Finished autocorrelation lecture
...
.hi-slate[1,500 100-period random walks]
```{r, walk1000, echo = F, cache = F}
# Set seed
set.seed(1246)
# Generate data
walk_df <- lapply(X = 1:1500, FUN = function(i) {
  tibble(x = cumsum(rnorm(1e2)), t = 1:1e2, walk = as.character(i))
}) %>% bind_rows()
# Plot
ggplot(data = walk_df, aes(x = t, y = x, color = walk)) +
...
.hi-slate[Granger and Newbold simulation example:] _t_ statistic ≈ `r lm(filter(walk_df, walk == 10)$x ~ filter(walk_df, walk == 12)$x) %>% summary %>% coef %>% extract(2,3) %>% round(2)`

```{r, gb12, echo = F}
ggplot(data = walk_df %>% filter(walk %in% c(10,12)), aes(x = t, y = x, color = walk)) +
  geom_hline(yintercept = 0, color = "grey85", size = 1.25) +
  geom_path() +
  scale_color_viridis(option = "magma", discrete = T, begin = 0.15, end = 0.85) +
  theme_axes_math
...
.hi-slate[Granger and Newbold simulation example:] _t_ statistic ≈ `r lm(filter(walk_df, walk == 13)$x ~ filter(walk_df, walk == 14)$x) %>% summary %>% coef %>% extract(2,3) %>% round(2)`

```{r, gb34, echo = F}
ggplot(data = walk_df %>% filter(walk %in% c(13:14)), aes(x = t, y = x, color = walk)) +
  geom_hline(yintercept = 0, color = "grey85", size = 1.25) +
  geom_path() +
  scale_color_viridis(option = "magma", discrete = T, begin = 0.15, end = 0.85) +
  theme_axes_math
...
.hi-slate[Granger and Newbold simulation example:] _t_ statistic ≈ `r lm(filter(walk_df, walk == 15)$x ~ filter(walk_df, walk == 16)$x) %>% summary %>% coef %>% extract(2,3) %>% round(2)`

```{r, gb56, echo = F}
ggplot(data = walk_df %>% filter(walk %in% c(15:16)), aes(x = t, y = x, color = walk)) +
  geom_hline(yintercept = 0, color = "grey85", size = 1.25) +
  geom_path() +
  scale_color_viridis(option = "magma", discrete = T, begin = 0.15, end = 0.85) +
  theme_axes_math
...
walk_pv <- lapply(X = seq(1, 1499, by = 2), FUN = function(i) {
