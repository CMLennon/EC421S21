...
subtitle: "EC 421, Metrics Review"
...
<center>
<img src="location_mapper.gif" width="720">
</center>
...
knitr::include_graphics("genart.png")
```
...
Our 95% confidence interval is thus $0.476 \pm 1.98 \times 0.0735 = \left[ 0.332,\, 0.620 \right]$

---

**Example:**
```{R ci r outputp2, echo = T, highlight.output = 5}
lm(y ~ x, data = pop_df) %>% tidy()
```

If we wanted a one-sided test we could access those results by simply dividing our p-values by 2
...
So we have a confidence interval for $\beta_1$, _i.e._, $\left[ 0.332,\, 0.620 \right]$.
...
--
 ...
 $t_\text{stat} = 6.4729$ and $t_\text{0.975, 28} = `r qt(0.975, 28) %>% round(2)`$
--
