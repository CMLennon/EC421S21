---
title: "Heteroskedasticity, Living in the Real World"
subtitle: "EC 421, 005"
...
- .hi[First assignment] due Next Week on Saturday (24 April.)
- Living with heteroskedasticity

## Upcoming

- .hi[Second assignment] "released" on Friday
- Midterm ~3 weeks away (May 6th - in class, no late/early exam available)
...
Given we so rarely know $h(x_i)$, we often wish to approximate it. We can do this with *feasible generalized least squares*. Rather than our normal WLS assumptions, we assume:

$$
Var(u|x) = \sigma^2e^{\delta_0 + \delta_1x_1 + \dots + \delta_kx_k}
$$

We can estimate our new, more flexible $h(x)$<sup>.pink[†]</sup> with a few clever transformations.

$$
u^2 = \sigma^2(e^{\delta_0 + \delta_1x_1 + \dots + \delta_kx_k})v
$$

.footnote[.pink[†] This is just one common form of FGLS - there are others] 
Where .hi[v] is assumed independent conditional on x.
---

Then, we can estimate the following equation using .hi[OLS]

--

$$log(e^2) = \hat{\alpha_0} + \hat{\delta_1}x_1 + \dots + \hat{\delta_k}x_k + e_{wls}$$

--

This allows us to recover $h(x)$ (so long as our assumptions were correct) by using $weights = \frac{1}{e^{\hat{\alpha_0} + \hat{\delta_1}x_1 + \dots + \hat{\delta_k}x_k}}$

--

Again: we assume

1. Our estimation of $h(x)$ gives good predictions
2. If it does not, we cannot say FGLS is **BLUE**.
3. In general, because we estimate $h(x)$ rather than know it, FGLS is not unbiased, but it is *consistent*. We'll talk about what that means in our next unit
